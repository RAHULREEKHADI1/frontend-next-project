...
const DataTypeStepper: React.FC<SidebarStepperProps> = ({ id }) => {
    console.log(id);

    const activeIndex = id - 1;
    const progressPercent = Math.round(((id) / STEPS.length) * 100);

    return (
        <aside className="flex flex-col justify-around min-h-screen w-full py-12 px-7.5 text-white font-sans bg-center bg-cover bg-no-repeat"
                    ...
                    ${isActive || isCompleted
                                    ...
                                    {!isLast && (
                                        <div className="flex px-3 py-2">
                                            <div className={`w-0.5 h-8 ${(isActive || isCompleted) ? "bg-[#4ade80]" : "bg-white"}`} />
                                        </div>
                                    )}
                                </div>
                            </div>
            ...
            <div className="mt-6">
                    <p className="text-sm text-blue-100/60 font-medium mb-1">
                        {activeIndex + 1} to {STEPS.length} step
                    </p>
                    <h3 className="text-[23px] font-bold mb-4">
                        {progressPercent}% to complete
                    </h3>
...
export default DataTypeStepper;
